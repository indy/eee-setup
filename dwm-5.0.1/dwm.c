...
/* enums */
enum { CurNormal, CurResize, CurMove, CurLock, CurLast };        /* cursor */
...
static void isg_toggle(const Arg *arg);
...
cleanup(void) {
  ...
  XFreeCursor(dpy, cursor[CurLock]);
...
setup(void) {
  ...
  cursor[CurLock] = XCreateFontCursor(dpy, XC_dot);
...
void
isg_toggle(const Arg *arg) {
     isglock = 1 - isglock;

//     XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
//                  None, cursor[isglock ? CurLock : CurNormal], CurrentTime);



//     if(isglock == 1) {
//          XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
//                       None, cursor[CurLock], CurrentTime);
//     } else {
//          XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
//                       None, cursor[CurNormal], CurrentTime);
//     }
